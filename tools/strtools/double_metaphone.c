/*
 * $Id: double_metaphone.c,v 2.1 2004/05/14 22:19:19 woodbri Exp $
 * 
 * This orginially came the the Perl String::DoubleMetaphone module.
 * It has been modified to return the first 4 digits of an numerical
 * string as the the metaphone code. It ignores leading zeros and 
 * returns "0" for strings of "0"s
 *
 */
#include <stdio.h>
#include <ctype.h>
...
DoubleMetaphone(char *str, char **codes)
{
    ...
    while ((primary->length < 4) || (secondary->length < 4))  
	  ...
	  switch (GetAt(original, current))
	    {
            case '0':
                if (primary->length == 0 && current < last) {
                    current += 1;
                    break;
                }
            case '1':
            ...
            case '8':
            case '9':
                digit[0] = GetAt(original, current);
                MetaphAdd(primary, digit);
                MetaphAdd(secondary, digit);
		current += 1;
		break;
