/*
iMaptools SQL Geocoder for Navteq data

Copyright 2013, Stephen Woodbridge DBA iMaptools.com, All Rights Reserved.

TODO:

1. add offset distance calculations  -- DONE
2. add queries and indexes so we can also search for city OR USPS city
3. look at query planner for degenerate cases where too many fields are absent
...
6. landmarks
7. parcels  -- we currently support Navteq pointaddress that are based on parcels
8. look into lex, gaz and rules for streets that did not standardize -- ON
...
DROP TYPE IF EXISTS public.stdaddr cascade;
...
*/
begin;
drop table if exists data.stdstreets cascade;

create table data.stdstreets (
...
insert into data.stdstreets (id,building,house_num,predir,qual,pretype,name,suftype,sufdir,ruralroute,extra,city,state,country,postcode,box,unit,name_dmeta)
    select id, (std).*, coalesce(nullif(dmetaphone((std).name),''), (std).name)
      from (
        select gid as id, standardize_address('lex'::text, 'gaz'::text, 'rules'::text,
               array_to_string(ARRAY[coalesce(substring(refaddr from '([0-9]+)$'), '123'::varchar), st_name], ' '),
               array_to_string(ARRAY[city,state,country,postcode], ',') ) as std
          from streets order by gid
       ) as foo;

commit;
-- Query returned successfully with no result in 10003434 ms.

/*
select count(*) from stdstreets where name_dmeta is null;  -- 21,029, Total query runtime: 722093 ms.
select count(*) from stdstreets where name_dmeta='';  -- 0, Total query runtime: 103937 ms.
*/
-- create indexes -----------------------------------------------------------------------------

create index stdstreets_name_city_state_idx on data.stdstreets using btree (name,state,city ASC NULLS LAST);
-- Query returned successfully with no result in 7755750 ms.

create index stdstreets_name_postcode_idx on data.stdstreets using btree (name,postcode ASC NULLS LAST);
-- Query returned successfully with no result in 8564460 ms.

create index stdstreets_name_dmeta_city_state_idx on data.stdstreets using btree (name_dmeta,state,city ASC NULLS LAST);
-- Query returned successfully with no result in 14762474 ms.

create index stdstreets_name_dmeta_postcode_idx on data.stdstreets using btree (name_dmeta,postcode ASC NULLS LAST);
-- Query returned successfully with no result in 5780911 ms.

begin;

drop table if exists streets_hn cascade;

create table streets_hn (
    gid integer not null primary key,
    refaddr integer,
    nrefaddr integer
);

insert into streets_hn
    select gid,
           substring(refaddr from '([0-9]+)$')::integer as refaddr,
           substring(nrefaddr from '([0-9]+)$')::integer as nrefaddr
    from streets;

commit;
-- Query returned successfully with no result in 6010841 ms.

select count(distinct zone_id) from streets;  -- 26006
select count(distinct area_id) from streets;  -- 28777
select count(distinct city||','||county||','||state) from streets;  -- 28812
...
insert into cities (city_dmeta, city, county, state, country, the_geom)
select coalesce(dmetaphone(city), city) as city_dmeta, city, county, state, country, st_centroid(st_collect(the_geom)) as the_geom
  from streets
 group by coalesce(dmetaphone(city), city), city, county, state, country;
commit;
-- Query returned successfully: 28812 rows affected, 37971301 ms execution time.

/*

-- Tried to use trigrams but not vary happy with the results.
...
vacuum analyze verbose data.stdstreets;
...
update stdstreets as a set building=b.building, house_num=b.house_num, predir=b.predir, qual=b.qual, pretype=b.pretype, name=b.name,
                            ...
                            country=b.country, postcode=b.postcode, box=b.box, unit=b.unit, name_dmeta=b.name_dmeta
   from (select *, dmetaphone(name) as name_dmeta from standardize_address(
        ...
        'select gid as id,
                array_to_string(ARRAY[coalesce(substring(refaddr from ''([[:digit:]]+)\$''), ''123''::varchar), st_name], '' '') as micro,
                 array_to_string(ARRAY[city,state,country,postcode], '','') as macro
           from streets
          where gid in (
                    select id
                      from stdstreets
          ...
          ) and array_to_string(ARRAY[coalesce(substring(refaddr from ''([[:digit:]]+)\$''), ''123''::varchar), st_name], '' '') is not null
          order by gid')) as b
  where a.id=b.id;
...
select count(*) 
      from data.stdstreets
     where coalesce(building, house_num, predir, qual, pretype, name, suftype, sufdir, ruralroute, extra, city, state, country, postcode, box, unit) is null;
-- 15,750, Total query runtime: 97949 ms.
...
insert into failures (cnt, name)
select count(*) as cnt, st_name as name
  from data.streets
 where gid in (
    select id
      from data.stdstreets
 ...
 group by name
 order by cnt desc;
-- Query returned successfully: 687 rows affected, 1443403 ms execution time.

/*
-- queries to check results of changes to lexicon and rules against the failures
...
begin
  ...
  return query select * from imt_geo_planner(rec);
end;
$body$
  language plpgsql immutable;
...
select a.*, 0 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name='WHITNEY' and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and case when a.suftype is not null then a.suftype='STREET' else true end and a.city='SAN FRANCISCO' and a.state='CALIFORNIA' and case when a.country is not null then a.country='USA' else true end and a.postcode='94131'
select a.*, 1 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name='WHITNEY' and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and case when a.suftype is not null then a.suftype='STREET' else true end and case when a.country is not null then a.country='USA' else true end and a.postcode='94131'
select a.*, 2 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name='WHITNEY' and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and case when a.suftype is not null then a.suftype='STREET' else true end and a.city='SAN FRANCISCO' and a.state='CALIFORNIA' and case when a.country is not null then a.country='USA' else true end
select a.*, 3 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name='WHITNEY' and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and a.city='SAN FRANCISCO' and a.state='CALIFORNIA' and case when a.country is not null then a.country='USA' else true end and a.postcode='94131'
select a.*, 4 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name='WHITNEY' and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and case when a.country is not null then a.country='USA' else true end and a.postcode='94131'
select a.*, 5 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name='WHITNEY' and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and a.city='SAN FRANCISCO' and a.state='CALIFORNIA' and case when a.country is not null then a.country='USA' else true end
select a.*, 6 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name_dmeta=coalesce(dmetaphone('WHITNEY'),'WHITNEY') and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and a.city='SAN FRANCISCO' and a.state='CALIFORNIA' and case when a.country is not null then a.country='USA' else true end and a.postcode='94131'
select a.*, 7 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name_dmeta=coalesce(dmetaphone('WHITNEY'),'WHITNEY') and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and case when a.country is not null then a.country='USA' else true end and a.postcode='94131'
select a.*, 8 as plan from stdstreets a, streets b, streets_hn c where a.id=b.gid and b.gid=c.gid and a.name_dmeta=coalesce(dmetaphone('WHITNEY'),'WHITNEY') and (97 between c.refaddr and c.nrefaddr or 97 between c.nrefaddr and c.refaddr ) and a.city='SAN FRANCISCO' and a.state='CALIFORNIA' and case when a.country is not null then a.country='USA' else true end
...
-- Total query runtime: 77293819 ms.
...
update intersections_tmp as a set
          cnt  = (select count(*) from st_vert_tmp b where b.vid=a.id),
          dcnt = (select count(distinct st_name) from st_vert_tmp b, streets c where b.gid=c.gid and b.vid=a.id and c.st_name is not null);
-- Query returned successfully: 17830507 rows affected, 57377601 ms execution time.
...
CREATE INDEX intersections_tmp_idx
  ...
  USING gist
  (the_geom);
-- Query returned successfully with no result in 5529419 ms. (for building three indexes)

select count(*) from intersections_tmp where dcnt>1;
-- 10,563,106  Total query runtime: 563034 ms.

delete from st_vert_tmp where vid in (select id from intersections_tmp where dcnt<2);
-- Query returned successfully: 34274421 rows affected, 115203918 ms execution time.

vacuum analyze st_vert_tmp;
-- Query returned successfully with no result in 3028448 ms.
...
insert into cities (city, city_dmeta, county, state, country, the_geom)
select city, dmetaphone(city) as city_dmeta, county, state, country, st_centroid(st_collect(the_geom)) as the_geom
  from streets
 group by city, dmetaphone(city), county, state, country;
commit;

CREATE INDEX cities_citydmeta_state_country_idx
  ON cities
  USING btree
  (city_dmeta COLLATE pg_catalog."default", state COLLATE pg_catalog."default", country COLLATE pg_catalog."default");

CREATE INDEX cities_city_state_country_idx
  ON cities
  USING btree
  (city COLLATE pg_catalog."default", state COLLATE pg_catalog."default", country COLLATE pg_catalog."default");

vacuum analyze cities;


create or replace function imt_geocode_place(city_in text, state_in text, country_in text, OUT city text, OUT state text, OUT country text, OUT loc geometry)
as $body$
declare
    rec record;
...
create or replace function imt_geocode_place(address text, OUT city text, OUT state text, OUT country text, OUT loc geometry)
as $body$
declare
    pa record;
    sa record;
...
/*
with z as (select upper('north chelmsford')::text as city_in, upper('massachusetts')::text as state_in, upper('usa')::text as country_in)
    select a.*, levenshtein(z.city_in, a.city)
      from cities a, z
     where a.city = z.city_in and a.state = z.state_in and case when z.country_in is not null then a.country = z.country_in else true end
     order by levenshtein(z.city_in, a.city) asc limit 1

with z as (select upper('north checlmsford')::text as city_in, upper('massachusetts')::text as state_in, upper('usa')::text as country_in)
    select a.*, levenshtein(z.city_in, a.city)
      from cities a, z
     where a.city_dmeta = dmetaphone(upper(city_in)) and a.state = z.state_in and case when z.country_in is not null then a.country = z.country_in else true end
     order by levenshtein(z.city_in, a.city) asc limit 1

select * from imt_geocode_place('north checlmsford ma');

*/

------------------------- postcode/zipcode geocoding ---------------------------------------------------------------
...
select * from imt_geocode_intersection('s stanworth dr', 'stanworth ln', 'princeton nj 08540', 1, 0);      -- FOUND
...
select * from imt_geocode_intersection('5th ave', '42nd street', 'new york ny 10036', 1, 0);


-------------------------------------------------------------------------------------------------

select * from imt_geocoder('11 radcliffe rd', 'north chelmsford, ma, 01863', 0, 10.0, 0);
select distinct * from streets a left outer join pointaddress b on a.link_id=b.link_id where a.link_id=22037362 and address='11';
...
select * from imt_geocoder('11 radcliff rd', 'north chelmsford, ma, 01863', 0, 0.0, 0);
...
select * from imt_geocoder('11 ratcliff rd', 'chelmsford, ma 01824', 1, 0.0, 0);                -- NOT FOUND
select * from imt_geocoder('11 ratcliff rd', 'north chelmsford, ma 01824', 1, 0.0, 0);
...
select * from imt_geocoder('7 Co Hwy 34', 'WILLIAMSPORT,PA,US,17701', 1, 0.0, 0);            -- FAILS
select * from imt_geocoder('1071 B Ave', 'Loxley, AL 36551', 1, 0.0, 0);                     -- FAILS in Tiger, google finds closest number ~3000
select * from imt_geocoder('1071 Chicago Ave', 'Loxley, AL 36551', 1, 0.0, 0);               -- looks like B Ave === Chicago Ave

select * from imt_geocoder('101 7th St', 'St. Paul, MN', 1, 0.0, 0);  -- Total query runtime: 138228 ms. 850 rows retrieved.
select * from imt_geocoder('101 7th St', 'Saint Paul, MN', 0, 0.0, 0);  -- Total query runtime: 1123 ms. 850 rows retrieved.
select * from imt_geocoder('101 7th St', 'St. Paul, MN 55102', 0, 0.0, 0);  -- Total query runtime: 219 ms. 16 rows retrieved.
...
select * from imt_geocode_place('2601 24TH AVE, ASTORIA NY 11102');
select * from imt_geocode_place('ASTORIA NY');
select * from imt_geocode_place('ASTORIA', 'NEW YORK', 'USA');

select * from imt_geocode_postcode('11102');
...
select id, (std).* from (
  select inp.id, standardize_address('lex'::text, 'gaz'::text, 'rules'::text, inp.micro, inp.macro) as std
    from (
      select 0::int4 as id, '116 I- 635'::text as micro, 'Coppell TX'::text as macro
      union all
      select 1::int4 as id, '116 I-635'::text as micro, 'Coppell TX'::text as macro) as inp ) as bar;

select * from parse_address('212 n 3rd ave, Minneapolis, mn 55401, USA');                    -- FIX THIS
select * from standardize_address('lex'::text, 'gaz'::text, 'rules'::text, 
        '212 n 3rd ave'::text,
...
select substring('1/23a45' from '(\\d+)$');

select * from streets limit 50;

select gid,
           substring(refaddr from '(\\d+)$')::integer as refaddr,
           substring(nrefaddr from '(\\d+)$')::integer as nrefaddr
    from streets limit 50

select * from imt_geocoder('RR 1', 'Langhorne PA 19047', 0, 0.0, 0);
select * from imt_geocoder('123 RR 1', 'Langhorne PA 19047', 0, 0.0, 0);


select * from parse_address('RR 1, Langhorne PA 19047');
-- "";"";"";"RR 1";"Langhorne";"PA";"19047";"";"US"
select * from parse_address('123 RR 1, Langhorne PA 19047');
-- "123";"RR 1";"";"123 RR 1";"Langhorne";"PA";"19047";"";"US"
...
--        'select seq, word::text, stdword::text, token from gaz union all select seq, word::text, stdword::text, token from lex ',
        'select seq, word::text, stdword::text, token from gaz order by id',
        'select * from rules order by id',
        'select 0::int4 as id, ''RR 1''::text as micro, ''Langhorne PA 19047''::text as macro');
-- 0;"";"";"";"";"";"";"";"";"RURAL ROUTE 1";"";"LANGHORNE";"PENNSYLVANIA";"";"19047";"";""
...
--        'select seq, word::text, stdword::text, token from gaz union all select seq, word::text, stdword::text, token from lex ',
        'select seq, word::text, stdword::text, token from gaz order by id',
        'select * from rules order by id',
        'select 0::int4 as id, ''123 RR 1''::text as micro, ''Langhorne PA 19047''::text as macro');

select * from parse_address('212 n 3rd ave, Minneapolis, mn 55401, USA');

select (geo).* from (
  select imt_geocoder(micro, macro, 1, 10, 0) as geo from (
    select address1 as micro, 
           array_to_string(ARRAY[city, state, zip, country], ',') as macro 
      from parse_address('212 n 3rd ave, Minneapolis, mn 55401')
  ) as std
) foo;

-- get a list of state names and abbreviations from gaz
-- might be useful for converting state names to abbreviations.

select * from gaz where token=11 and length(word)=2 and id not in (248,450) order by stdword

-----------------------------------------------------------------------------------------------------------
-- create zipcode centroids based on street data
-- WE DO NOT NEED TO DO THIS WE HAVE NAVTEQ ZIPCODE FILES
/*
drop table if exists postcode cascade;
create table postcode (
  postcode character varying(11) not null primary key
);
SELECT addGeometryColumn('postcode', 'the_geom', 4326, 'POINT', 2);

insert into postcode
  select postcode, st_centroid(st_collect(the_geom)) as the_geom
    from streets
...
select * from standardize_address('lex'::text, 'gaz'::text, 'rules'::text, '123 AR-7', 'boston, ma' );
