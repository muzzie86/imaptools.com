...
insert into data.stdstreets (id,building,house_num,predir,qual,pretype,name,suftype,sufdir,ruralroute,extra,city,state,country,postcode,box,unit,name_dmeta)
    select *, coalesce(nullif(dmetaphone(name),''), name) as name_dmeta from standardize_address(
        ...
        'select gid as id,
           ...
           from streets
          where gid between $i and $j      -- <<<######### THIS WILL NOT RUN HERE, it is from the perl script ##############
          order by gid');

commit;

/*
select count(*) from stdstreets where name_dmeta is null;  -- 223,284
select count(*) from stdstreets where name_dmeta='';  -- 13,958,506
select * from stdstreets where name_dmeta='' limit 10;

-- Fix the original insert <sigh>
update stdstreets set name_dmeta=name where name_dmeta='';
-- Query returned successfully: 13958506 rows affected, 17627303 ms execution time.
...
-- 213,283, Total query runtime: 672355 ms.

select count(*) as cnt, st_name as micro
  from data.streets
 where gid in (
    select id
      from data.stdstreets
 ...
 group by micro
 order by micro;
-- Total query runtime: 781092 ms. 5754 rows retrieved.
...
--raise notice 'id: %, num: %, aoff: %', id, num, aoff;
  ...
  select b.refaddr, b.nrefaddr, a.side, st_geometryn(a.the_geom,1) as the_geom, c.the_geom as point
    ...
    from data.streets_hn b, 
         data.streets a 
         left outer join data.pointaddress c on a.link_id=c.link_id and substring(c.address from '([[:digit:]]+)$')::integer = to_number(num,'999999999')
...
-- Total query runtime: 41269197 ms.
...
-- Query returned successfully: 16800226 rows affected, 20826381 ms execution time.

select count(*) from intersections_tmp where dcnt>1;
-- 10,086,140  Total query runtime: 202003 ms.

delete from st_vert_tmp where vid in (select id from intersections_tmp where dcnt<2);
-- Query returned successfully: 32149021 rows affected, 54667945 ms execution time.
...
drop if exists geo_intersection cascade;
...
begin
  -- add a house number here because it is required for standardize_address(), we will remove it later
  sql := 'select 0::int4 as id, '||trim(both from quote_literal('10 '||micro1_in))||'::text as micro, '||trim(both from quote_literal(macro_in))||'::text as macro
    union all
select 1::int4 as id, '||trim(both from quote_literal('10 '||micro2_in))||'::text as micro, '||trim(both from quote_literal(macro_in))||'::text as macro';

--raise notice 'sql: %', sql;
  for srec in select * from standardize_address(
                 'select seq, word::text, stdword::text, token from lex order by id',
                 'select seq, word::text, stdword::text, token from gaz order by id',
                 'select * from rules order by id',
                 sql)
  loop
      -- kill house_num now because we don't want it for intersections
      srec.house_num := null;
      
      -- make sure we got a result that we can use
--raise notice 'srec: %', srec;
...
select substring('1/23a45' from E'(\\d+)$');
