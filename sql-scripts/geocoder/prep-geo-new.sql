...
~/work/pagc/pagc/branches/sew-refactor/pagclib/api/psql/prep-geo.pl    -- old script to standardizer tiger data
~/work/pagc/pagc/branches/sew-refactor/pagclib/api/psql/mk-stdaddr-db  -- old script(pg8.3) to create a database stdaddr
~/work/pagc/pagc/branches/sew-refactor/pagclib/api/psql/load-pagc2geo  -- script to load processed tiger files
~/work/pagc/pagc/branches/sew-refactor/pagclib/api/psql/geocoder-design.txt
~/work/pagc/pagc/branches/sew-refactor/postgresql/*                    -- new address standardizer package for postgresql
...
/*  old way of doing things before write 
insert into stdstreets (id,building,house_num,predir,qual,pretype,name,suftype,sufdir,ruralroute,extra,city,state,country,postcode,box,unit)
...
-- this is now done in the insert above
-- update stdstreets set name_dmeta=coalesce(dmetaphone(name), name) where name is not null and length(dmetaphone(name_dmeta))=0;
-- Query returned successfully: 50320063 rows affected, 2106821 ms execution time.

-- clean up and fix the last update the messed things up
-- update stdstreets set name_dmeta=nullif(coalesce(dmetaphone(name), name),'') where length(name_dmeta)=0;
-- Query returned successfully: 12372409 rows affected, 6786641 ms execution time.
...
/*
-- update standardizations after updating lexicon or gazeteer
-- drop the stdstreets table and rebuild it, don't try incremental updates
*/
...
select count(*) from streets where ac2='MA' and coalesce(predirabrv, pretypabrv, prequalabr, name, sufdirabrv, suftypabrv, sufqualabr) is null;
...
--raise notice 'sfld: %', sfld;
  ...
  if coalesce(sfld.house_num,sfld.predir,sfld.qual,sfld.pretype,sfld.name,sfld.suftype,sfld.sufdir,sfld.city,sfld.state,sfld.postcode,sfld.country) is not null then
...
--raise notice 'doing for rec in select ...';
...
select * from standardize_address('lex', 'gaz', 'rules', '11 radcliffe rd', 'north chelmsford, ma, 01863');
select * from standardize_address('data.lex', 'data.gaz', 'data.rules', '11 radcliffe rd', 'north chelmsford, ma, 01863');
select * from standardize_address('"data"."lex"', '"data"."gaz"', '"data"."rules"', '11 radcliffe rd', 'north chelmsford, ma, 01863');
