...
BEGIN
        LOOP
                ...
                IF NOT FOUND THEN
                        INSERT INTO intersections_tmp (the_geom) VALUES (st_force_2d(point));
                ELSE
                        EXIT;
                END IF;
        END LOOP;
...
-- Total query runtime: 33703827 ms.

update intersections_tmp as a set
    cnt  = (select count(*) from st_vert_tmp b where b.vid=a.id),
    dcnt = (select count(distinct name) from st_vert_tmp b, streets c where b.gid=c.gid and b.vid=a.id and c.name is not null);
-- Query returned successfully: 37312802 rows affected, 16010209 ms execution time.
...
-- Query returned successfully with no result in 795739 ms.


select * from imt_reverse_geocode_flds(SetSRID(MakePoint(-122.035464,47.601621), 4326));
select address, coalesce(village,city) as city, state, postcode, distance from imt_reverse_geocode_flds(SetSRID(MakePoint(-122.035464,47.601621), 4326));
*/
